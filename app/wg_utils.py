...
def get_wg_version() -> str:
    """Get WireGuard version info"""
    try:
        version = run_cmd(["wg", "--version"])
        return version
    except Exception:
        return "WireGuard Version: Unknown"
...
def add_peer(name: str) -> Dict:
    ...
    try:
        run_cmd(["wg", "set", "wg0", "peer", pub_key, "preshared-key", "/dev/stdin", "allowed-ips", f"{ip_v4}/32,{ip_v6}/128"], input_data=psk)
    except Exception as e:
        logger.warning(f"Failed to update live wg interface: {e}")
...
def remove_peer(peer_id: str) -> bool:
    ...
    try:
        run_cmd(["wg", "set", "wg0", "peer", peer["public_key"], "remove"])
    except Exception as e:
        logger.warning(f"Failed to remove peer from live interface: {e}")
...
def generate_client_config(peer: Dict) -> str:
    """Generate exact WireGuard/Wiresock config syntax"""
